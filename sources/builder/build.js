...
setTimeout( function()
{
    ...
    .then(result => {
        console.log( result);
        FS.writeFileSync( 'md/documentation/cache/fr/topics.txt', result.text );
    } )
    .catch( err => {
        console.log( 'Error on: md/documentation/cache/fr/topics.txt' );
        console.error( err )
        process.exit();
    } );
}, 500 );
...
function readDirectory( path, depth )
{
    ...
    filenames.forEach( file => {
        ...
        else
        {
            if( name != '' )
            {
                if( file.name != '000_chapter.txt' && file.name != 'topics.txt' )
                {
                    var page = file.name;
                    // var file_cache = page.strReplace( ' ', '_' );
                    // file_cache = page.strReplace( '/', '_' );
                    ...
                    setTimeout( function()
                    {
                        ...
                        .then(result => {
                            console.log( result);
                            FS.writeFileSync( 'md/documentation/cache/fr/' + file.name, result.text );
                        } )
                        .catch( err => {
