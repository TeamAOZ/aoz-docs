...
var docPath = rootDir + 'md/documentation/default/syntax';
var categoriesEN = readDirectory( docPath, 0 );

var docPath = rootDir + 'md/documentation/default/more_tricks';
...
//var docPath = rootDir + 'md/documentation/default/user_guide/table_of_contents';
var docPath = rootDir + 'md/documentation/default';
...
setTimeout( function()
{
    ...
    .then(result => {
        FS.writeFileSync( rootDir + 'md/documentation/cache/fr/default/topics.txt', result.text );
    } )
    .catch( err => {
        ...
        else
        {
            if( name != '' )
            {
                if( PATH.extname( file.name ) == '.txt' && file.name != 'topics.txt' && file.name != '000_chapter.txt' )
                {
                    ...
                    var targetPath = path.strReplace( rootDir + 'md/documentation', '' );
                    ...
                    for( var pl = 0; pl < listPaths.length; pl++)
                    {
                        if( listPaths[ pl ] != '' )
                        {
                            ...
                            if( !FS.existsSync( rootDir + 'md/documentation/cache/fr' + tPath ) )
                            {
                                FS.mkdirSync( rootDir + 'md/documentation/cache/fr' + tPath );
                            }
                        }
                    }
                    /**
                    if( !FS.existsSync( rootDir + 'md/documentation/cache/fr' + tPath + '/000_chapter.txt' ) )
                    {
                        FS.writeFileSync( rootDir + 'md/documentation/cache/fr' + tPath + '/000_chapter.txt', '# ' + name );
                    }
                    */
                   ...
                   /**
                    setTimeout( function()
                    {
                        ...
                        //var file_cache = page.strReplace( ' ', '_' );
                        //file_cache = page.strReplace( '/', '_' );
                        if( !FS.existsSync( rootDir + 'md/documentation/cache/fr' + tPath + '/' + file.name ) )
                        {
                            var data = FS.readFileSync( path + '/' + file.name );
                            translator.translateText( data.toString(), null, 'fr', { tagHandling: 'xml', ignoreTags: [ 'keep' ] } )
                            .then(result => {
                                console.log( result);
                                FS.writeFileSync( rootDir + 'md/documentation/cache/fr' + tPath + '/' + file.name, result.text );
                            } )
                            .catch( err => {
